...
class Transform:
    ...
    calling transform(<array>). The class allows for basic transformations as well as derived 
    transformations (such as rotations around the origin).
    
    To use, initialize the Transformation matrix then call the functions associated with each 
    type of desired linear transformation (in order). Then call T.transform(A), where A is the 
    matrix to transform. These same transformations can be applied to more than one matrix.
    ...
    ```python
        ...
        T = Transform() #Initialize Transformation matrix
        T.rotateAroundZ(0.79) #Rotate pi/4 radians
        T.translate(1.2, 1.2) #Translate 1.2 units up and right
        transformed_triangle = T.transform(triangle) #Apply transformation to input matrix
        T = Transform() #Clear Transformation matrix
    ```
